...
function create_stair_fix(world, p, a, count) {
  local m = 584;
  local l = 0.4;
  local x = -l * sin(a);
  local y =  l * cos(a);
  for(local i = 0; i < count; i++) {
    sfixb <- CreateObject( m, world, p + Vector(x * i, y * i, 0.0), 255);
    sfixb.RotateToEuler(Vector(0,0,a), 0);
  }
}

function create_double_stair_fix(world, p, a, count, d) {
  create_stair_fix(world, p, a, count)
  create_stair_fix(world, p + Vector(d, 0.0, 0.0), a, count)
}
...
function create_double_stair_up_down_fix(world, p, a, d) {
  ...
  create_double_stair_fix(world, p1, a, 2, d);

  local p2 = Vector(p.x - sin(a) * d_forward, p.y + cos(a) * d_forward, p.z + d_level);
  create_double_stair_fix(world, p2, a + PI, 3, d);
}

function onScriptLoad() {

  local world = 0 
  local d = 1.924; // Distance between 2 stairs
