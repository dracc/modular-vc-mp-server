...
function create_escalator_fix(world, p, a, count) {
  ...
  for(local i = 0; i < count; i++) {
    fixed_escalator_step <- CreateObject( m, world, p + Vector(x * i, y * i, 0.0), 255);
    fixed_escalator_step.RotateToEuler(Vector(0,0,a), 0);
  }
}
...
function create_double_escalator_up_down_fix(world, p, a, d) {
...
function onScriptLoad() {
  ...
  // 5   6
  //
  // Coordinates reflect the bottom of the left side

  local p1 = Vector(406.122 - 0.05, 1193.38, 17.9824);
  create_double_escalator_up_down_fix(world, p1, 0.0, d);

  local p2 = Vector(419.754 - 0.1, 1193.32, 17.8704);
  create_double_escalator_up_down_fix(world, p2, 0.0, d);

  local p3 = Vector(414.661 - 0.02, 1145.71, 17.4724 + 0.04);
  create_double_escalator_up_down_fix(world, p3, PI, -d);

  local p4 = Vector(412.785 - 0.1 + 0.05 + d, 1102.65, 17.4724);
  create_double_escalator_up_down_fix(world, p4, 0.0, -d);

  local p5 = Vector(408.213 - 0.1, 1058.5 - 0.02, 17.4724 + 0.71);
  create_double_escalator_up_down_fix(world, p5, PI, -d);

  local p6 = Vector(421.778 - 0.05, 1058.52, 18.2419 - 0.25);
  create_double_escalator_up_down_fix(world, p6, PI, -d);
